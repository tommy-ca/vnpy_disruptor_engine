...
try:
    ...
    from vnpy_disruptor_engine import create_engine
...
def run_hft():
    ...
    if isinstance(event_engine, DisruptorEventEngine):
        print("Engine Type: DISRUPTOR")
        print(f"Strategy:    {SETTINGS['event.wait_strategy']}")
        print(f"Buffer Size: {SETTINGS['event.buffer_size']}")
    else:
        print(
            "CRITICAL: Failed to initialize Disruptor Engine. Falling back to Standard Queue."
        )
        sys.exit(1)
