...
def run_bench(name, engine_factory):
    ...
    try:
        ...
        return {
            ...
            "p50": p50,
            "p99": p99,
            "tput_single": tput_single,
            "tput_batch": tput_batch,
        }
...
def main():
    ...
    print("\n" + "=" * 85)
    print(
        f"{'Strategy':<30} | {'P50 (µs)':<10} | {'P99 (µs)':<10} | {'TPS (Single)':<12} | {'TPS (Batch)':<12}"
    )
    print("-" * 85)
    for r in results:
        if r:
            ...
            print(
                f"{r['name']:<30} | {r['p50']:<10.1f} | {r['p99']:<10.1f} | {r['tput_single']:>12,.0f} | {batch_str:>12}"
            )
    print("=" * 85)
