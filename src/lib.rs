...
use disruptor::{
    build_multi_producer, wait_strategies::WaitStrategy, BusySpin, BusySpinWithSpinLoopHint,
    MultiProducer, ProcessorSettings, Producer, Sequence, SingleConsumerBarrier,
};
...
pub struct VnpyEvent {
    pub data: Mutex<Option<Arc<PyObject>>>,
}
...
impl InnerProducer {
    fn try_publish(&mut self, _py: Python<'_>, data: PyObject) -> Result<(), ()> {
        ...
        match self {
            InnerProducer::BusySpin(Some(p)) => p
                .try_publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                })
                .map(|_| ())
                .map_err(|_| ()),
            InnerProducer::BusySpinHint(Some(p)) => p
                .try_publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                })
                .map(|_| ())
                .map_err(|_| ()),
            InnerProducer::Yielding(Some(p)) => p
                .try_publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                })
                .map(|_| ())
                .map_err(|_| ()),
            InnerProducer::Sleeping(Some(p)) => p
                .try_publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                })
                .map(|_| ())
                .map_err(|_| ()),
            InnerProducer::Adaptive(Some(p)) => p
                .try_publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                })
                .map(|_| ())
    ...
    fn publish(&mut self, data: PyObject) -> bool {
        let arc_data = Arc::new(data);
        match self {
            InnerProducer::BusySpin(Some(p)) => {
                p.publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                });
                true
            }
            InnerProducer::BusySpinHint(Some(p)) => {
                p.publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                });
                true
            }
            InnerProducer::Yielding(Some(p)) => {
                p.publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                });
                true
            }
            InnerProducer::Sleeping(Some(p)) => {
                p.publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                });
                true
            }
            InnerProducer::Adaptive(Some(p)) => {
                p.publish(|s| {
                    *s.data.lock() = Some(Arc::clone(&arc_data));
                });
                true
    ...
    fn try_publish_batch(
        ...
        match self {
            InnerProducer::BusySpin(Some(p)) => p
                .try_batch_publish(n, |iter| {
                    for (s, a) in iter.zip(arc_events.iter()) {
                        *s.data.lock() = Some(Arc::clone(a));
                    }
                })
                .map(|_| ())
                .map_err(|_| Vec::new()),
    ...
    fn publish_batch(&mut self, events: Vec<PyObject>) -> bool {
        ...
        match self {
            InnerProducer::BusySpin(Some(p)) => {
                p.batch_publish(n, |iter| {
                    for (s, a) in iter.zip(arc_events) {
                        *s.data.lock() = Some(a);
                    }
                });
                true
...
impl DisruptorProducer {
        ...
        match wait_strategy {
            ...
            _ => {
                return Err(pyo3::exceptions::PyValueError::new_err(format!(
                    "Unknown wait_strategy: {wait_strategy}"
                )))
            }
        }
    ...
    pub fn try_publish(&self, py: Python<'_>, data: PyObject) -> PyResult<bool> {
        ...
        let producer = guard
            .as_mut()
            .ok_or_else(|| pyo3::exceptions::PyRuntimeError::new_err("Stopped"))?;
    ...
    fn check_error(&self, py: Python<'_>) -> PyResult<()> {
        ...
        if let Some(exc_obj) = guard.take() {
            return Err(PyErr::from_value(exc_obj.into_bound(py)));
        }
        Ok(())
    }
    ...
    pub fn start_worker(
        ...
        let events_buffer: Arc<Mutex<Vec<Arc<PyObject>>>> =
            Arc::new(Mutex::new(Vec::with_capacity(ADAPTIVE_BATCH_SIZE)));
        ...
        let handler = move |event: &VnpyEvent, _sequence: Sequence, end_of_batch: bool| {
            ...
            if let Some(arc_data) = event.data.lock().take() {
                events_buffer.lock().push(arc_data);
            }
            if end_of_batch {
                Python::with_gil(|py| {
                    ...
                    let list_data: Vec<PyObject> =
                        buffer.iter().map(|arc| (**arc).clone_ref(py)).collect();
                    if let Ok(list) = PyList::new(py, list_data) {
                        if let Err(e) = callback.call1(py, (list,)) {
                            *last_error.lock() =
                                Some(e.into_pyobject(py).unwrap().into_any().unbind());
                    ...
                    buffer.clear(); // Safe drop within GIL
                });
            }
        };
        ...
        if producer_guard.is_some() {
            return Err(pyo3::exceptions::PyRuntimeError::new_err("Already started"));
        }
        self.active.store(true, Ordering::Release);

        match self._wait_strategy.as_str() {
            "busy_spin" => {
                let mut b = build_multi_producer(self._buffer_size, factory, BusySpin)
            ...
            "busy_spin_hint" => {
                let mut b =
                    build_multi_producer(self._buffer_size, factory, BusySpinWithSpinLoopHint)
                        .handle_events_with(handler);
                if let Some(id) = core_id {
                    b = b.pin_at_core(id);
                }
            ...
            "yielding" => {
                let mut b = build_multi_producer(self._buffer_size, factory, Yielding)
                    .handle_events_with(handler);
                if let Some(id) = core_id {
                    b = b.pin_at_core(id);
                }
            ...
            "sleeping" => {
                let mut b = build_multi_producer(self._buffer_size, factory, Sleeping)
                    .handle_events_with(handler);
                if let Some(id) = core_id {
                    b = b.pin_at_core(id);
                }
            ...
            "blocking" => {
                ...
                let mut b = build_multi_producer(self._buffer_size, factory, strategy)
                    .handle_events_with(handler);
                if let Some(id) = core_id {
                    b = b.pin_at_core(id);
                }
    ...
    pub fn wait_strategy(&self) -> String {
        self._wait_strategy.clone()
    }
